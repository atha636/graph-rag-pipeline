...
import type { Document, View, DocFilter, SessionStats } from '../types';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  onDeleteDoc, isOnline, theme, onToggleTheme, stats,
}) => {
      ...
      <aside style={{ ...styles.sidebar, width: 56, minWidth: 56 }}>
        ...
        <nav style={{ ...styles.nav, padding: '12px 8px', alignItems: 'center' }}>
          {navItems.map(item => (
            ...
            >
              <span style={{ color: currentView === item.id ? 'var(--accent)' : 'var(--sidebar-text)' }}>
                {item.icon}
              </span>
            </button>
          ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '0 0 16px' }}>
          ...
          <button
            style={{ ...styles.collapseBtn, transform: 'none' }}
    ...
    <aside style={styles.sidebar}>
      ...
      <div style={styles.logo}>
        ...
        <div style={styles.logoActions}>
          <div title={isOnline ? 'Backend connected' : 'Backend offline'} style={styles.statusWrap}>
            {isOnline
      ...
      <div style={styles.docsSection}>
        ...
        {/* Filter tabs — only show when there are mixed types */}
        {documents.length > 0 && (
          <div style={styles.filterTabs}>
            {DOC_FILTERS.filter(f => f.id === 'all' || filterCounts[f.id] > 0).map(f => (
              <button
                ...
                style={{
                  ...styles.filterTab,
                  ...(docFilter === f.id ? styles.filterTabActive : {}),
                }}
                onClick={() => setDocFilter(f.id)}
              >
                {f.label}
                {f.id !== 'all' && (
                  <span style={styles.filterCount}>{filterCounts[f.id]}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div style={styles.docsList}>
          {filteredDocs.length === 0 && !loadingDocs && (
            <div style={styles.emptyDocs}>
              ...
              <p style={{ fontSize: 10.5 }}>Upload Multiple files to get started.</p>
            </div>
          )}
          ...
          {filteredDocs.map(doc => (
            <div
              ...
              style={{
                ...styles.docItem,
                background: hoveredDoc === doc.id ? 'var(--sidebar-item-hover)' : 'transparent',
              }}
            ...
            >
              ...
              <div style={styles.docMeta}>
                ...
                {doc.uploadedAt && (
                  <span style={styles.docDate}>{doc.uploadedAt.slice(0, 10)}</span>
                )}
              </div>
              {hoveredDoc === doc.id && (
                <button
      ...
      <div style={styles.footer}>
        ...
        >
          ...
          <span style={styles.statsToggleText}>Session Stats</span>
          <span style={{ ...styles.statsToggleText, marginLeft: 'auto' }}>
            {showStats ? '▲' : '▼'}
          </span>
        </button>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  logoText: {
    fontSize: 14, fontWeight: 700, color: 'var(--sidebar-text-active)',
    letterSpacing: '-0.3px', flex: 1,
  },
  logoActions: { display: 'flex', alignItems: 'center', gap: 6 },
  statusWrap:  { display: 'flex', alignItems: 'center', gap: 3 },
  statusLabel: { fontSize: 10, fontWeight: 600 },
  collapseBtn: {
    background: 'none', border: 'none', cursor: 'pointer',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    padding: 3, borderRadius: 5, opacity: 0.6,
  ...
  sectionLabel: {
    fontSize: 9.5, fontWeight: 700, color: 'var(--text-faint)',
    letterSpacing: '0.1em', padding: '0 8px 7px', display: 'block',
  },
  navItem: {
  ...
  filterTabs: {
    display: 'flex', gap: 3, padding: '0 8px 6px', flexShrink: 0,
  },
  filterTab: {
    display: 'flex', alignItems: 'center', gap: 3,
    fontSize: 10.5, fontWeight: 500, padding: '3px 7px',
  ...
  filterCount: {
    fontSize: 9.5, background: 'rgba(255,255,255,0.08)',
    padding: '0 4px', borderRadius: 3, color: 'var(--text-muted)',
  },
  docsList: {
  ...
  emptyDocs: {
    ...
    gap: 5, padding: '20px 8px', color: 'var(--text-muted)',
  ...
  deleteBtn: {
    ...
    opacity: 0.7, padding: 2, borderRadius: 4, flexShrink: 0,
  },
